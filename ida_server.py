...
def create_server(port):
    global host
    return SimpleXMLRPCServer((host, port), logRequests=True, allow_none=True)

def create_server_multiple(start_port):
    print("Starting from port {}, trying to create server there.".format(start_port))
    for port in range(start_port, start_port + 10):
        try:
            ret = create_server(port)
            return ret, port
        except Exception as e:
            print("Had error when trying to create server at port: {}, trying next port: {}".format(port, e))
...
def startup():
    ...
    global thread
    global port
    server, actual_port = create_server_multiple(port)
    register_module(idc)
    ...
    print('IDA Pro xmlrpc hosted on http://%s:%s' % (host, actual_port))
